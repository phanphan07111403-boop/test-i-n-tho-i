# Wild Rift 7.3 — forum + winrate meta, 5 outstanding champs per role

Patch **7.3** shipped **2026-09-21**. Season 23 started **2026-09-23**. This is a week-one snapshot: CN ranked numbers plus Challenger guides and VN / EN patch discussion.

## Sources

| Source | What it is | Date / patch |
|--------|------------|--------------|
| [Official 7.3 notes](https://wildrift.leagueoflegends.com/en-us/news/game-updates/wild-rift-patch-notes-7-3/) | System changes, new items, champion kits | 2026-09-21 |
| [WildRiftFire stats](https://www.wildriftfire.com/stats) | CN ranked WR / PR / BR | labeled **Patch 7.3** |
| [Rift Patch Notes winrates](https://www.riftpatchnotes.com/winrates) | CN ranked, daily | labeled **Patch 7.3** |
| [Wild Rift Core](https://wildriftcore.com/en/rankings/) | Tencent CN | updated **2026-09-22** |
| [WildRiftFire 7.3 tier list](https://wildriftfire.com/tier-list) | iTzSTU4RT Challenger list | Patch 7.3 |
| [WildRiftFire champion guides](https://www.wildriftfire.com/) | Live 7.3 cores + runes | Patch 7.3 |
| [VNG Tốc Chiến forum](https://community.vnggames.com/news/wr/tom-tat-toc-chien-patch-7-3-phan-1-dai-tiec-am-nhac-c-ng-loat-thay-doi-moi.html) | VN community patch recap | 2026-09-10 preview + live discussion |

Reddit `/r/wildrift` is still mostly **pre-7.3** (Magnetic Blaster threads). Live build talk is on WRFire, CN ladder, and VNG.
...
7.3 is not a small balance patch. It splits **crit snipers** from **on-hit clump** ADCs, then the rest of the map reacts to that.

### ADC identity split (the big change)

- **Crit damage 175% → 200%.** Attack-speed cap **2.5 → 3.0**.
- **Magnetic Blaster deleted.** Range / waveclear / burst / kite are no longer one item.
- New first-items:
  - **Yun Tal Wildarrows** (3100) — AD + AS, crit stacks over time.
  - **Hexoptics C44** (2900) — 55 AD / 25% crit, range on takedown, damage from distance.
  - **Fiendhunter Bolts** — Zeal that spikes **after you ult** (Twitch / Zeri / Yunara).
  - **Stormrazor** stays as the immediate Energized slow.
- **Statikk Shiv reworked** for on-hit: 40 AD / 40 AP / 30% AS, lightning **copies on-hit onto bounce targets**. Not a crit item.

Caitlyn, Ashe, Lucian, Twitch, Kai'Sa, etc. were retuned so crit abilities scale with crit chance/damage. Senna / Jhin / Yasuo / Yone had their crit modifiers cut so they do not explode with 200% crit.

**Result:** Dragon lane jumped. Caitlyn went from ~49.6% WR / ~28% pick in 7.2 to **~52.2% WR / ~36.5% pick**. Miss Fortune (old Magnetic-Blaster queen) dropped to **49.0% WR** with still-high pick.

### Jungle

Smite now adds **true-damage DoT** and a **stat matching the jungler's identity**. Epic monsters are tankier, so full-clear farmers (Lillia, Amumu, Jarvan) and anti-ADC tanks (Rammus) beat early invade duelists. Lee Sin is **47.0% WR / 14.7% pick** — still popular, no longer strong.

New champs in this cycle: **Hwei** (mid, already live), **Sylas**, **Rek'Sai** (Rek'Sai still filling in on the ladder).

### Support / top react to ADC

Anti-auto tanks and peel are overperforming: **Rammus 58.7%**, **Malphite** (top and support), **Shen 56.4%**, **Senna 55.9%**. Enchanters that buff the new crit ADC (Sona, Janna) are also up. Hook champs (Blitz, Pyke) did not get the same bump.

### Mid

Control mages that sit behind the new ADC and delete the other one are the lane: **Syndra 55.3% / 33% ban**, **Hwei 51.5% / 13% pick / 32% ban**. Assassins and Yasuo are the losers.

---

## Outstanding builds being discussed

Three ADC lanes. Do not mix them.

| Lane | Core | Who |
|------|------|-----|
| **Crit sniper** | Hexoptics / Stormrazor / Yun Tal → **Infinity Edge** → RFC / LDR | Caitlyn, Ashe, Smolder |
| **Ult-window carry** | Kraken or Yun Tal → Runaan / **Fiendhunter** → IE | Yunara, Twitch, Zeri |
| **On-hit clump** | **Statikk Shiv** (or Kraken) → Rageblade → Terminus / Nashor. Runaan 4th **only** if the fight is still a clump | Kog'Maw, Varus, Kalista, Kai'Sa, Teemo |

Other roles:

| Build | Why people are talking about it |
|-------|----------------------------------|
| **Senna Shiv → RFC → LDR → IE** | Q applies on-hit; 7.3 Shiv bounce + souls. WRFire S+ support core. |
| **Sona Tear → Helia → Diadem / Harmonic** | New enchanter toys (Helia, Harmonic Echo, Diadem of Songs). Gold-efficient aura for the ADC patch. |
| **Zyra Liandry → Rylai → Cryptbloom** | Plant burn still banned at **37%**. Rylai keeps the 3s tick on target. |
| **Malphite Iceborn → Thornmail → Frozen Heart → Twinguard** | Anti-crit frontline. Played **top and support**. |
| **Rammus Thornmail → Sunfire → Dawnshroud → Twinguard** | Ice Overlord. Highest WR champion in the game because ADCs auto him. |
| **Hwei Blackfire → Infinity Orb → Deathcap** | New mid. 13% pick / 32% ban already. |
| **Syndra Luden → Infinity Orb → Deathcap** | Same burst page, now even better at deleting the 36%-pick Caitlyn. |
| **Lillia Liandry → Blackfire → Deathcap** | Smite + burn matches the new jungle. |

---

## ADC — 5 outstanding

Caitlyn is the patch. Ashe has the cleanest WR among marksmen. Yunara / Smolder are the contested scalers. Kai'Sa is the on-hit answer that still gets picked.

### 1. Caitlyn — S+ — 52.2% WR / 36.5% pick / 7.1% ban

**Why:** Base AD 54→60, Headshot and Ace in the Hole now scale with **crit chance and crit damage**, trap Headshots add bonus AD damage. Highest pick rate in any role.

**Build (WRFire 7.3):** Long Sword → Berserker's → **Hexoptics C44** → **Infinity Edge** → **Rapid Firecannon** → LDR → Bloodthirster / Galeforce.

**Runes:** Lethal Tempo · Brutal · Cut Down · Legend: Alacrity · Bone Plating. Flash + Ghost.
...
**Build:** Berserker's → **Stormrazor** → **Runaan's Hurricane** → **Infinity Edge** → LDR → Bloodthirster.
...
**Why:** S+ on the Challenger list, second-most-picked ADC after Caitlyn, banned like a new champ. 7.3 shaved a bit of passive magic damage and Q AS, but ult-window still matches **Fiendhunter**.

**Build:** Berserker's → **Kraken Slayer** → **Runaan's Hurricane** → **Infinity Edge** → LDR → Stormrazor.

**Discussed alt:** Yun Tal or Hexoptics first, **Fiendhunter** around the ult. WRFire keeps Kraken → Runaan as the default.
...
## Support — 5 outstanding

Senna is running the role. Malphite support is the anti-ADC cheese. Sona / Janna buff the new carry. Zyra is the perma-ban mage.

### 1. Senna — S+ — 55.9% WR / 7.8% pick / 1.4% ban

**Why:** Highest support win rate. 7.3 cut Mist crit (15%→10% per 20 souls) and put her on **90% crit damage**, and she still jumped. Q applies on-hit, so **new Shiv** is the item people argue about.

**Build (WRFire 7.3 support):** Spectral Sickle → Berserker's / Dynamism → **Statikk Shiv** → **Rapid Firecannon** → **LDR** → Infinity Edge → Galeforce.

**Runes:** Fleet Footwork · Font of Life · Bone Plating · Perseverance · Brutal. Flash + Heal.
...
**Build:** Relic Shield → Steelcaps → **Zeke's Convergence** → Thornmail → Iceborn → Twinguard.
...
**Why:** Not the highest WR. Highest **ban** in support (Morgana 48% is mid+support combined). Plant burn into Caitlyn lane is miserable to play against.

**Build:** Sickle → Mana boots → **Liandry's Torment** → **Rylai's** → Cryptbloom → Zhonya.

**Runes:** Aery or Comet · Manaflow · Transcendence · Scorch · Bone Plating.
...
## Jungle — 5 outstanding

Anti-ADC tanks and reliable full-clears. Assassin junglers (Lee, Yi, Graves, Viego) are the losers.

### 1. Rammus — S+ — **58.7% WR** / 4.0% pick / 4.1% ban

**Why:** Best win rate in the game. Caitlyn / Yunara / Ashe auto him, Thornmail + Curl prints the game.

**Build:** Ruby → Steelcaps → **Thornmail** → **Sunfire Aegis** → Dawnshroud → **Amaranth's Twinguard** → Mantle.
...
**Why:** Only jungler on WRFire **S+** with a real ban rate. Paranoia deletes the 36%-pick Caitlyn before the fight starts. New Smite helps his clear.

**Build:** Long Sword → Dynamism → **Duskblade** → **Serpent's Fang** → **Serylda's Grudge** → Sundered Sky → GA.
...
Delete the enemy ADC, or be Galio and eat the engage. Hwei is the discussion; Syndra is the problem.

### 1. Syndra — S+ — 55.3% WR / 11.5% pick / **33.4% ban**

**Why:** Highest mid WR, first or second ban. 7.2d already buffed Q/W/E; 7.3 just gave her more squishy ADCs to execute.

**Build:** Amp Tome → Mana boots → **Luden's Echo** → **Infinity Orb** → **Rabadon's Deathcap** → Void → Zhonya.
...
**Build:** Amp Tome → Mana boots → **Blackfire Torch** → **Infinity Orb** → Deathcap → Void → Zhonya.

**Runes:** Arcane Comet · Manaflow · Transcendence · Scorch · Bone Plating. Flash + Barrier.
...
## Top (Baron) — 5 outstanding

Tanks that ignore the 200% crit ADC, plus Ambessa as the fighter who still works.

### 1. Malphite — S+ — **57.6% WR** (WRFire Baron) / 7.3% pick / 28.4% ban

**Why:** Best top on WRFire. Armor + Ground Slam vs the entire Dragon-lane overhaul. Ult is still the most important button in ranked.

**Build:** Ruby → Steelcaps → **Iceborn Gauntlet** → **Thornmail** → **Frozen Heart** → Twinguard → Kaenic Rookern.
...
## Quick ban priority (7.3 week one)

| Priority | Champ | Role | Why |
|----------|-------|------|-----|
| 1 | Cho'Gath / Morgana / Master Yi | mixed | 41–58% ban, leftover + tank |
| 2 | Syndra | mid | 33% ban, 55% WR |
| 3 | Hwei | mid | 32% ban, new |
| 4 | Nocturne | jungle | 32% ban, deletes ADC |
| 5 | Malphite | top/sup | 26–28% ban |
| 6 | Zyra | sup/mid | 37% ban |
| 7 | Yunara / Smolder | ADC | if Caitlyn is first-pick |

---

## Caveats

- CN ladder, first days of 7.3 / S23. Rates will move.
- Low-pick high-WR (Shen, Amumu, Olaf, Janna, Kog) is real but easy to ban or dodge.
- WRFire guide **prose** sometimes still names Magnetic Blaster. Trust the **7.3 item list** on the same page (Hexoptics, Shiv, Helia, Twinguard).
- Rek'Sai / Sylas need another week of games before they belong on these fives.
